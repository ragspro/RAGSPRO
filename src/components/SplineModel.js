'use client'
import { useEffect, useRef } from 'react'

export default function SplineModel() {
  const robotRef = useRef()
  
  useEffect(() => {
    const robot = robotRef.current
    if (!robot) return
    
    const handleMouseMove = (e) => {
      const rect = robot.getBoundingClientRect()
      const x = (e.clientX - rect.left - rect.width / 2) / rect.width
      const y = (e.clientY - rect.top - rect.height / 2) / rect.height
      
      robot.style.transform = `rotateY(${x * 10}deg) rotateX(${-y * 10}deg)`
    }
    
    robot.addEventListener('mousemove', handleMouseMove)
    robot.addEventListener('mouseleave', () => {
      robot.style.transform = 'rotateY(0deg) rotateX(0deg)'
    })
    
    return () => {
      robot.removeEventListener('mousemove', handleMouseMove)
    }
  }, [])
  
  return (
    <div className="w-full h-full bg-gradient-to-br from-accent/5 to-purple-600/5 rounded-2xl flex items-center justify-center border border-accent/10 relative overflow-hidden">
      {/* Animated background particles */}
      <div className="absolute inset-0">
        {[...Array(20)].map((_, i) => (
          <div
            key={i}
            className="absolute w-1 h-1 bg-accent/30 rounded-full animate-pulse"
            style={{
              left: `${Math.random() * 100}%`,
              top: `${Math.random() * 100}%`,
              animationDelay: `${Math.random() * 2}s`,
              animationDuration: `${2 + Math.random() * 2}s`
            }}
          />
        ))}
      </div>
      
      {/* 3D Robot */}
      <div 
        ref={robotRef}
        className="relative transition-transform duration-300 ease-out cursor-pointer"
        style={{ transformStyle: 'preserve-3d' }}
      >
        {/* Robot Body */}
        <div className="relative">
          {/* Head */}
          <div className="w-16 h-16 bg-gradient-to-br from-accent to-accent/80 rounded-lg mx-auto mb-2 relative animate-bounce" style={{ animationDuration: '3s' }}>
            {/* Eyes */}
            <div className="absolute top-4 left-3 w-2 h-2 bg-white rounded-full animate-pulse"></div>
            <div className="absolute top-4 right-3 w-2 h-2 bg-white rounded-full animate-pulse"></div>
            {/* Antenna */}
            <div className="absolute -top-2 left-1/2 transform -translate-x-1/2 w-1 h-4 bg-accent"></div>
            <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 w-3 h-3 bg-neon rounded-full animate-ping"></div>
          </div>
          
          {/* Body */}
          <div className="w-20 h-24 bg-gradient-to-br from-gray-700 to-gray-800 rounded-xl mx-auto mb-2 relative border border-accent/30">
            {/* Chest Panel */}
            <div className="absolute top-3 left-1/2 transform -translate-x-1/2 w-12 h-8 bg-accent/20 rounded border border-accent/50">
              <div className="w-full h-1 bg-accent animate-pulse mt-2"></div>
              <div className="w-3/4 h-1 bg-accent/60 animate-pulse mt-1 mx-auto"></div>
            </div>
            
            {/* Side Panels */}
            <div className="absolute top-6 left-1 w-2 h-4 bg-accent/40 rounded animate-pulse"></div>
            <div className="absolute top-6 right-1 w-2 h-4 bg-accent/40 rounded animate-pulse"></div>
          </div>
          
          {/* Arms */}
          <div className="absolute top-16 -left-6 w-4 h-12 bg-gradient-to-b from-gray-600 to-gray-700 rounded-full animate-pulse" style={{ animationDelay: '0.5s' }}></div>
          <div className="absolute top-16 -right-6 w-4 h-12 bg-gradient-to-b from-gray-600 to-gray-700 rounded-full animate-pulse" style={{ animationDelay: '1s' }}></div>
          
          {/* Legs */}
          <div className="flex justify-center gap-2">
            <div className="w-5 h-16 bg-gradient-to-b from-gray-700 to-gray-800 rounded-full animate-pulse" style={{ animationDelay: '1.5s' }}></div>
            <div className="w-5 h-16 bg-gradient-to-b from-gray-700 to-gray-800 rounded-full animate-pulse" style={{ animationDelay: '2s' }}></div>
          </div>
          
          {/* Feet */}
          <div className="flex justify-center gap-2 mt-1">
            <div className="w-6 h-3 bg-accent/60 rounded-full"></div>
            <div className="w-6 h-3 bg-accent/60 rounded-full"></div>
          </div>
        </div>
        
        {/* Floating Elements */}
        <div className="absolute -top-8 -left-8 w-4 h-4 border-2 border-accent rounded-full animate-spin" style={{ animationDuration: '4s' }}></div>
        <div className="absolute -top-4 -right-8 w-3 h-3 bg-neon rounded-full animate-bounce" style={{ animationDelay: '1s' }}></div>
        <div className="absolute -bottom-4 -left-6 w-2 h-2 bg-purple-500 rounded-full animate-ping" style={{ animationDelay: '2s' }}></div>
      </div>
      
      {/* Glow Effect */}
      <div className="absolute inset-0 bg-gradient-radial from-accent/5 via-transparent to-transparent animate-pulse" style={{ animationDuration: '4s' }}></div>
    </div>
  )
}